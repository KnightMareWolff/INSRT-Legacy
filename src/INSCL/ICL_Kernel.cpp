...
void CICLKernel::PreparaParametro(cl_context pICL_DeviceContext, IGLVertexBuffer *pGLBuffer, uint pGLBufferSize, CIMTMatriz *pTransfMatrix)
{
    cl_int  ICL_Error;
    ...
    if(!ICL_VertexBuffer)
    {
       ICL_VertexBuffer = new IGLVertexBuffer[pGLBufferSize];
    }
    else
    {
    ...
    memcpy( ICL_VertexBuffer, pGLBuffer ,pGLBufferSize * sizeof(IGLVertexBuffer));

    //Cria um buffer no dispositivo(Buffer de entrada)
    ICL_BufferIn = clCreateBuffer(pICL_DeviceContext,
                                  CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  sizeof(IGLVertexBuffer) * pGLBufferSize,
                                  ICL_VertexBuffer,
                                  &ICL_Error);
...
void CICLKernel::PreparaParametro(cl_context pICL_DeviceContext, uint pGLBufferId, uint pGLBufferSize, CIMTMatriz *pTransfMatrix)
{
    cl_int  ICL_Error;
...
void CICLKernel::ExecutaKernel(cl_command_queue pICL_DeviceCommandQueue, IGLVertexBuffer *pGLBuffer)
{
   cl_int  ICL_Error;
   ...
   ICL_Error = clEnqueueReadBuffer(pICL_DeviceCommandQueue,
                                   ...
                                   sizeof(IGLVertexBuffer) * ICL_GLBufferSize, // how much to copy
                                   pGLBuffer,
                                   0,
                                   NULL,
                                   eventND);
