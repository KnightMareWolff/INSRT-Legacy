...
void CICLDevice::execPgm(IGLVertexBuffer *pICL_Buffer, uint pICL_BufferSize, CIMTMatriz *pICL_Transform)
{
   CICLPrograma *tPrograma = PegaProgramaPorNome("INSGL.cl");
   if(tPrograma)
   {
...
void CICLDevice::execPgm(uint pICL_BufferId, uint pICL_BufferSize, CIMTMatriz *pICL_Transform)
{
    CICLPrograma *tPrograma = PegaProgramaPorNome("INSGL");
    if(tPrograma)
    {
       CICLKernel *tKernel = tPrograma->PegaKernelPorNome("TransformGLBuffer");
       if(tKernel)
...
CICLPrograma* CICLDevice::PegaProgramaPorNome(QString pICL_Nome)
{
...
bool CICLDevice::CompilaKernels()
{
    ...
    switch(ICL_SystemDeviceType)
    {
       case ICL_DEV_CPU:
       {
          ...
          QDir dir = QDir(CIUTGerenciadorPath::IUT_PathWorld + "Kernels/AMD/CPU/");
          ...
          for(cl_uint i=0;i<ICL_QtdProgramas;i++)
          {
             ...
             ICL_Programas.push_back(new CICLPrograma(ICL_DeviceContext,ICL_DeviceID,dir.path() + "/" + entries[i]));
          }
       }break;
       ...
       {
          //Pega a lista de Kernels disponiveis
          QDir dir = QDir(CIUTGerenciadorPath::IUT_PathWorld + "Kernels/AMD/GPU/");
          QStringList nameFilters;
          nameFilters << QStringLiteral("*.cl");
          QStringList entries = dir.entryList(nameFilters,QDir::Files|QDir::Readable, QDir::Size);
